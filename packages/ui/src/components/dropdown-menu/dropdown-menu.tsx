...
const Root = React.forwardRef<ViewRef, SlottableViewProps & { onOpenChange?: (open: boolean) => void }>(
...
const Content = React.forwardRef<PressableRef, SlottablePressableProps & PositionedContentProps>(
	(
		...
		const { open, onOpenChange, nativeID, triggerPosition, setTriggerPosition, contentLayout, setContentLayout } =
			useRootContext();
...
const CheckboxItem = React.forwardRef<PressableRef, SlottablePressableProps & DropdownMenuCheckboxItemProps>(
	(
		{
			asChild,
			checked,
...
const RadioItem = React.forwardRef<PressableRef, SlottablePressableProps & DropdownMenuRadioItemProps>(
	(
		{ asChild, value: itemValue, textValue, onPress: onPressProp, disabled = false, closeOnPress = true, ...props },
		ref,
	) => {
...
const SubTrigger = React.forwardRef<PressableRef, SlottablePressableProps & DropdownMenuSubTriggerProps>(
